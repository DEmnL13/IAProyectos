# Práctica 1 Final/Definitivo/ultra/hiper/chido/supreme/uwu
import pygame
import random
import heapq
...
# Crear laberinto
def generate_maze():
    maze = [[Node(x, y, walkable=random.random() > 0.3) for y in range(ROWS)] for x in range(COLS)]
    maze[0][0].walkable = True  # Entra
    maze[COLS - 1][ROWS - 1].walkable = True  # Sale
    return maze

# Aqui se conectan los vecinos
...
def reconstruct_path(end):
    ...
    path.reverse()
    for node in path:
        node.draw(BLUE)  # Resalta la ruta + corta sisisisi
    pygame.display.update()
    return path
